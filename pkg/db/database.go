...
func (db *Database[T, ID]) Create(ctx context.Context, entity T) (T, error) {
	var empty T
	if db.storage == nil {
		cestlog.Debug("[cestdb.Create] storage layer is nil")
		return empty, cesterr.F("cestdb.Create err: storage is nil")
	}

	// insert into PostgresSQL first
	createdEntity, err := db.storage.Create(ctx, entity)
	if err != nil {
		cestlog.Debugf("[cestdb.Create] storage err: %v", err)
		return empty, cesterr.F("cestdb.Create err: %v", err)
	}

	// if search layer is nil, return created entity
	if db.search == nil {
		cestlog.Infof("[cestdb.Create] search => pass")
	...
	if err != nil {
		cestlog.Warnf("[cestdb.Create] search err: %v", err)
	}
...
func (db *Database[T, ID]) Update(ctx context.Context, id ID, entity T) (T, error) {
	var empty T
	if db.storage == nil {
		cestlog.Debug("[cestdb.Update] storage layer is nil")
		return empty, cesterr.F("cestdb.Update err: storage is nil")
	}

	// update in PostgresSQL
	updatedEntity, err := db.storage.Update(ctx, id, entity)
	if err != nil {
		cestlog.Debugf("[cestdb.Update] storage err: %v", err)
		return empty, cesterr.F("cestdb.Update storage err: %v", err)
	}

	// if search layer is nil, return updated entity
	if db.search == nil {
		cestlog.Info("[cestdb.Update] search => pass")
	...
	if existed, err := db.search.Exists(ctx, id); err == nil && existed {
		...
		if err != nil {
			cestlog.Warnf("[cestdb.Update] search err: %v", err)
		}
	}
...
func (db *Database[T, ID]) Delete(ctx context.Context, id ID) error {
	if db.storage == nil {
		cestlog.Debug("[cestdb.Delete] err: storage is nil")
		return cesterr.F("cestdb.Delete err: storage is nil")
	}

	if db.search == nil {
		cestlog.Info("[cestdb.Delete] search => pass")
	}

	if db.search != nil {
		// delete from Elasticsearch, if error, return error
		if esErr := db.search.Delete(ctx, id); esErr != nil {
			cestlog.Debugf("[cestdb.Delete][search] err: %v", esErr)
			return cesterr.F("cestdb.Delete search err: %v", esErr)
		}
	}

	// after delete from search, delete from storage
	if err := db.storage.Delete(ctx, id); err != nil {
		cestlog.Debugf("[cestdb.Delete][storage] err: %v", err)
		return cesterr.F("cestdb.Delete storage err: %v", err)
	}
...
func (db *Database[T, ID]) Get(ctx context.Context, id ID) (T, error) {
	...
	if db.storage == nil {
		cestlog.Debug("[cestdb.Get] storage layer is nil")
		return empty, cesterr.F("cestdb.Get err: storage is nil")
	}

	if db.search == nil {
		cestlog.Info("[cestdb.Get] search => pass")
	}
	...
	if db.search != nil {
		...
		cestlog.Warnf("[cestdb.Get][search] err: %v", err)
	}

	data, err := db.storage.Get(ctx, id)
	if err != nil {
		cestlog.Debugf("[cestdb.Get][storage] err: %v", err)
		return empty, cesterr.F("cestdb.Get storage err: %v", err)
	}
...
func (db *Database[T, ID]) GetAll(ctx context.Context) ([]T, error) {
	if db.storage == nil {
		cestlog.Debug("[cestdb.GetAll] err: storage is nil")
		return nil, cesterr.F("cestdb.GetAll err: storage is nil")
	}

	if db.search == nil {
		cestlog.Info("[cestdb.GetAll] search => pass")
	}
	...
	if db.search != nil {
		...
		cestlog.Warnf("[cestdb.GetAll][search] err: %v", err)
	}

	resp, err := db.storage.GetAll(ctx)
	if err != nil {
		cestlog.Debugf("[cestdb.GetAll][search] err: %v", err)
		return nil, cesterr.F("cestdb.GetAll search err: %v", err)
	}
...
func (db *Database[T, ID]) Exists(ctx context.Context, id ID) (bool, error) {
	if db.storage == nil {
		cestlog.Debug("[cestdb.Exists] storage layer is nil")
		return false, cesterr.F("cestdb.Exists storage layer is nil")
	}

	if db.search == nil {
		cestlog.Info("[cestdb.Exists] search => pass")
	}
	...
	if db.search != nil {
		...
		cestlog.Warnf("[cestdb.Exists] search err: %v", err)
	}

	existed, err := db.storage.Exists(ctx, id)
	if err != nil {
		cestlog.Debugf("[cestdb.Exists] storage err: %v", err)
		return false, cesterr.F("cestdb.Exists storage err: %v", err)
	}
...
func (db *Database[T, ID]) Count(ctx context.Context) (int64, error) {
	if db.storage == nil {
		cestlog.Debug("[cestdb.Count][storage] layer is nil")
		return -1, cesterr.F("cestdb.Count storage layer is nil")
	}
	...
	if db.search != nil {
		...
		cestlog.Warnf("[cestdb.Count][search] err: %v", err)
	}

	count, err := db.storage.Count(ctx)
	if err != nil {
		cestlog.Debugf("[cestdb.Count][storage] err: %v", err)
		return -1, cesterr.F("cestdb.Count storage err: %v", err)
	}
